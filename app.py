...
def process_data(input_file_path):
    ...
    if not month_orders.empty:
        total_orders_val = len(month_orders.groupby(['Mã KH', month_orders['Ngày đặt'].dt.date]))
    else:
        total_orders_val = 0
        
    if not month_calls.empty:
        total_calls_val = len(month_calls.groupby(['Mã KH', month_calls['Thời gian checkin'].dt.date]))
    else:
        total_calls_val = 0
